...
.. tip::
    ...
    回傳型別和函式名稱放在同一行。如果放得下的話，參數也盡量放在同一行。放不下的話，參數列就依 :ref:`函式呼叫 <function-calls>` 的方式分行。
...
.. tip::
    ...
    你可以一行寫完函式呼叫，也可以將括號內的參數分行，或是將參數放到下一行並且加上 4 格的縮排。如果沒有其它顧慮的話，盡可能精簡行數，比如把多個參數適當地放在同一行裡。
...
注意事項：
    ...
    - 所有基礎類別名稱應在 80 個字元的限制下盡量與子類別名稱放在同一行。
